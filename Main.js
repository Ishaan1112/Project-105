...
function gotResult(error, results) {
  if (error) {
    ...
    console.log(results);
    document.getElementById("reuult_object_name").innerHTML = results[0].label;
    document.getElementById("reuult_object_accuracy").innerHTML = results[0].confidence.toFixed(3);
...
classifier = ml5.imageClassifier('https://teachablemachine.withgoogle.com/models/v_sl95BzE/model.json',modelLoaded);
  ...
  function check()
  {
    img = document.getElementById('captured_imaje');
    classifier.classify(img, gotResylt);
  }
